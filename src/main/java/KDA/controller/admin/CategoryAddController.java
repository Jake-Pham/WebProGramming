...
public class CategoryAddController extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5646102160619147332L;
	CategoryService cateService = new CategoryServiceImpl();

	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

		RequestDispatcher rq = req.getRequestDispatcher("/views/admin/add-category.jsp");
		rq.forward(req, resp);
	}
		...
		try {
			...
			for (FileItem item : items) {
				if (item.getFieldName().equals("cname")) {
					...
					category.setIcons(item.getString());
